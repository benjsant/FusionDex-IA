...
function PokedexContent() {
  ...
  const listQuery   = usePokemonList({ page, page_size: PAGE_SIZE, type_id: typeId });
  ...
  // Quand on cherche, on garde l'intersection recherche × type côté client
  // (l'endpoint /search ne prend pas de type_id).
  const filtered = useMemo(() => {
    if (!isSearching || !typeId) return pokemons;
    const target = types.find((t) => t.id === typeId);
    if (!target) return pokemons;
  ...
  }, [pokemons, typeId, types, isSearching]);
